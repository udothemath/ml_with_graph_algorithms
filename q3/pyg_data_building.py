...
# from troch_geometric.nn import HeteroConv
...
class HeteroGNN(torch.nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict):
        for i, conv in enumerate(self.convs):
            ...
            if i % 2 == 0: 
                edge_dict = dict([
                    (edge_type, tensor) for edge_type, tensor in edge_index_dict.items() if edge_type[0] == 'cust'])
            else:
                edge_dict = dict([
                    (edge_type, tensor) for edge_type, tensor in edge_index_dict.items() if edge_type[0] == 'prod'])
