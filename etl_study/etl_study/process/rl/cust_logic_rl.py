...
class CustLogic(CommonLogic):
    ...
    def calculate_first_level_rate(self, travel_card, five_profession_card,
        ...
        """
        第一段利率為一次撥付型末段利率(calculate_total_rate)
        ...
        Args:
            ...
            military_police_firefighters(str): 是否行業別為軍警消
            salary_ind(str): 是否為本行薪轉戶
            pd_value(numeric)：PD值
            lgd(numeric)
            pd_grade(numeric)：PD等級

        Returns(float):
            first_level_rate: 第一段利率
        """
        first_level_rate = self.calculate_total_rate(travel_card,
                                                     five_profession_card,
                                                     world_card, wm_cust,
                                                     gov_employee,
                                                     military_police_firefighters,
                                                     salary_ind,
                                                     pd_value, lgd, pd_grade)
    ...
    def run_all(self, with_cc, krm040_ind, bam087_ind, krm001_ind, jas002_ind,
        ...
        # interest_rate_1取至小數第四位
        round_interest_rate_1 = '{:.4f}'.format(round(interest_rate_1, 4))
        # interest_rate_1小數第四位調整為 8
        interest_rate_1 = float(str(round_interest_rate_1)[:-1] + '8')
