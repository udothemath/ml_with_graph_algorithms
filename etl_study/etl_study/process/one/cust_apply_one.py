...
class CustApply(ForkingBase):
    def __init__(self, input_table_name='cust_datamart' + TABLE_SUBFIX,
                 target_table_name='cust_tmp_result_one' + TABLE_SUBFIX, group_id=-1, verbose=False):
    ...
    def logic_object(self):
        ...
        return CustLogic
        """
        cust_logic = import_module('src.process.one.cust_logic_one')
    ...
    def check_target_table(self, schema_name, table_name):
        ...
        assert row_count > 0
        assert self._check_group_interest_rate_1_fee_amount(
    ...
    def _check_group_interest_rate_1_fee_amount(
        ...
        assert all(table.loc[table['group'] == '01',
                   'interest_rate_1'].isin([0.0138]))

        assert all(table.loc[table['group'] == '02',
                   'interest_rate_1'].isin([0.0168]))

        assert all(table.loc[table['group'] == '03',
                   'interest_rate_1'].isin([0.0168]))

        assert all(table.loc[table['group'] ==
                   '01', 'fee_amount'].isin([5000]))

        assert all(table.loc[table['group'] ==
                   '02', 'fee_amount'].isin([6000]))

        assert all(table.loc[table['group'] ==
                   '03', 'fee_amount'].isin([6000]))
    ...
    def _check_apdlv(self, schema_name, table_name):
        ...
        assert all(round(table['apdlv'], 5)
                   .isin([-1, 0.0015, 0.00225, 0.00338, 0.00506,
    ...
    def _check_lgd(self, schema_name, table_name):
        ...
        '''
        table = self.select_table('feature', sqlstring)
        assert all(round(table['lgd'], 3).isin([-1, 0.441, 0.458, 0.462,
                                                0.513, 0.53, 0.535,
                                                0.598, 0.652]))
        return True
    ...
    def _check_pre_net_income(self, schema_name, table_name):
        ...
        '''
        table = self.select_table('feature', sqlstring)
        assert table['min_pre_net_income'][0] >= -1
        return True
    ...
    def _check_max_limit(self, schema_name, table_name):
        ...
        assert table['max_max_limit'][0] <= 3000000
        return True
    ...
    def _check_period(self, schema_name, table_name):
        ...
        '''
        table = self.select_table('feature', sqlstring)
        assert all(table['period_1'].isin([3]))
        assert all(table['period_2'].isin([57]))
        return True
